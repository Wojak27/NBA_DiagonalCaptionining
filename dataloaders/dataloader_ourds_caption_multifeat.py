...
class OURDS_Caption_DataLoader(Dataset):
    ...
    def __init__(
        ...
        # valid_nn = 4465
        # test_nn = 4465
        split_dict = json.load(open('./data/split2video_id_after_videos_combination.json', 'r'))
    ...
    def _get_allplayer_feature(self, choice_video_ids, feature_dict, gameid2videoid, max_frames, feature_size, max_players):
        ...
        video_mask = np.zeros((len(choice_video_ids), max_frames * max_players), dtype=np.longlong)
        max_video_length = [0] * len(choice_video_ids)

        video = np.zeros((len(choice_video_ids), max_frames * max_players, feature_size), dtype=np.float_)
        for i, video_id in enumerate(choice_video_ids):

            if self.fine_tune_extractor == False:
                
                if os.path.exists(os.path.join(
                        feature_dict, video_id + '.npy'
                    )):
                    video_slice = np.load(
                        os.path.join(
                            feature_dict, video_id + '.npy'
    ...
    def _get_feature(self, choice_video_ids, feature_dict, max_frames, feature_size):
        ...
        '''
        video_mask = np.zeros((len(choice_video_ids), max_frames), dtype=np.longlong)
        max_video_length = [0] * len(choice_video_ids)

        video = np.zeros((len(choice_video_ids), self.max_frames, feature_size), dtype=np.float_)
        for i, video_id in enumerate(choice_video_ids):
    ...
    def _get_video(self, choice_video_ids):
        ...
        video = np.zeros((len(choice_video_ids), self.max_frames, self.feature_size), dtype=np.float_)
        for i, video_id in enumerate(choice_video_ids):
    ...
    def _get_bbx(self, choice_video_ids):
        ...
        video = np.zeros((len(choice_video_ids), self.max_frames, self.feature_size_bbx), dtype=np.float_)
        for i, video_id in enumerate(choice_video_ids):
    ...
    def _get_court(self, choice_video_ids):
        ...
        video = np.zeros((len(choice_video_ids), self.max_frames, self.feature_size_court), dtype=np.float_)
        for i, video_id in enumerate(choice_video_ids):
