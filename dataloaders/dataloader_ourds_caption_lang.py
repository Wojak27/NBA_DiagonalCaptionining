...
class OURDS_Caption_Lang_DataLoader(Dataset):
    ...
    def _get_text(self, video_id, caption, information_text):
        k = 1
        ...
        for i, video_id in enumerate(choice_video_ids):
            words = self.tokenizer.tokenize(information_text)
            words = ["[CLS]"] + words
    ...
    def __getitem__(self, idx):
        ...
        context_player_data = " ; ".join(list(set([t for t in caption.replace("_", "_ ").replace("(", " ").replace(",","").replace(")", " ").replace(":","").replace("side", " ").split() if t.startswith("PLAYER")])))
        
        if "PLAYER" in context_player_data:
            for player in context_player_data.split(";"):
                player = player.strip()
                playerInfo = self.playerIDdict[player.replace("PLAYER", "").strip()]
                caption = caption.replace(player, " " + playerInfo["first_name"] + " " + playerInfo["last_name"] + " ")
                context_player_data = context_player_data.replace(player, " " + playerInfo["first_name"] + " " + playerInfo["last_name"])
        
        pairs_text, pairs_mask, pairs_segment, \
        pairs_masked_text, pairs_token_labels, \
        pairs_input_caption_ids, pairs_decoder_mask, \
        pairs_output_caption_ids, choice_video_ids, task_type = self._get_text(video_id, caption, context_player_data)
