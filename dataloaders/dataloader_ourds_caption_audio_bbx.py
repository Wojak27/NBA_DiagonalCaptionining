...
class OURDS_Caption_Audio_BBX_DataLoader(Dataset):
    ...
    def __init__(
        ...
        if self.fine_tune_extractor == False:
            ...
            self.audio_feature_dict = audio_feature
    ...
    def _get_audio(self, choice_audio_ids):
        ...
        # Mask Frame Model <-----
        video_labels_index = [[] for _ in range(len(choice_audio_ids))]
        masked_video = audio.copy()
        for i, video_pair_ in enumerate(masked_video):
            for j, _ in enumerate(video_pair_):
                if j < max_audio_length[i]:
                    ...
                    # mask token with 15% probability
                    if prob < self.mask_prob:
                        masked_video[i][j] = [0.] * audio.shape[-1]
                        video_labels_index[i].append(j)
                    else:
                        video_labels_index[i].append(-1)
                else:
        ...
        return audio, audio_mask, masked_video, video_labels_index
    ...
    def __getitem__(self, idx):
        ...
        audio, audio_mask, masked_video, video_labels_index = self._get_audio([video_id])
        ...
        return pairs_text, pairs_mask, pairs_segment, video, video_mask, \
               ...
               pairs_input_caption_ids, pairs_decoder_mask, pairs_output_caption_ids,task_type, audio, audio_mask, bbx, bbx_mask
