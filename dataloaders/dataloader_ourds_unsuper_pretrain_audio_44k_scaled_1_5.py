...
class OURDS_PreTrainAudio44k_scaled_DataLoader(Dataset):
    ...
    def __init__(
            ...
            mask_prob=0.15,
            extra_feat_annotation = None,
            audio_dim = 128,
    ):
        assert extra_feat_annotation is not None
        extra_feat_annotation = json.load(open(extra_feat_annotation, 'r'))
        self.extra_feat_annotation = [x for x in extra_feat_annotation]
        ...
        if self.fine_tune_extractor == False:
            ...
            self.extra_feat_annotation = [x for x in self.feature_dict if "video" not in x]
        ...
        try:
            self.videoid2gameid_eventid = json.load(open('/home/karolwojtulewicz/code/NSVA/data/videoid2gameid_eventid.json','r'))
        except:
        ...
        # valid_nn = 4465
        # test_nn = 4465
        split_dict = json.load(open('/home/karolwojtulewicz/code/NSVA/data/split2video_id_after_videos_combination.json', 'r'))
        ...
        if split_type == "train":  # expand all sentence to train
            ...
            for vidx,vid in enumerate(self.extra_feat_annotation):
                self.sentences_dict[len(self.sentences_dict)] = (vid, "<T3>_"+self.extra_feat_annotation[vidx])
    ...
    def _get_video(self, choice_video_ids):
        ...
        n_tokens = int(self.max_frames*(1-self.mask_prob))
        trimmed_video = np.zeros((len(choice_video_ids), n_tokens, self.feature_size))
        for i, video_pair_ in enumerate(masked_video):
            ...
            #         if prob < (max_video_length[i]/self.max_frames)*self.mask_prob:
            ...
            #     else:
            #         video_labels_index[i].append(-1)
            unmasked_index = random.sample(range(max_video_length[i]), n_tokens)
            unmasked_index.sort()
            trimmed_video[i] = video_pair_[unmasked_index]
            
            for j, _ in enumerate(video_pair_):
                if j < max_video_length[i]:
                    if j not in unmasked_index:
                        masked_video[i][j] = [0.] * video.shape[-1]
                        video_labels_index[i].append(j)
                    else:
                        video_labels_index[i].append(-1)
        ...
        # -----> Mask Frame Model
        masked_video = masked_video > 0
        zeros = np.zeros((masked_video.shape[0], masked_video.shape[1]))
        zeros[masked_video[:,:,0] > 0] = 1
        masked_video = zeros
        return trimmed_video, video_mask, masked_video, video_labels_index,video

    def _get_audio(self, choice_audio_ids):
        audio_mask = np.zeros((len(choice_audio_ids), self.max_frames), dtype=int)
        max_audio_length = [0] * len(choice_audio_ids)

        audio = np.zeros((len(choice_audio_ids), self.max_frames, self.audio_dim))
        for i, audio_id in enumerate(choice_audio_ids):

            if self.fine_tune_extractor == False:
                audio_slice = self.bbx_feature_dict[audio_id]
            # else:
            #     gameid_eventid = self.videoid2gameid_eventid[video_id]
            #     # gameid = gameid_eventid.split('-')[0]
            #     # eventid = gameid_eventid.split('-')[1]
            ...
            if slice_shape[0] > 8 and random.random() > 0.5:
                # make the audio shorter with random trim between 5 and slice_shape[0] - 1
                audio_slice = audio_slice[:random.randint(5, slice_shape[0] - 1)]  
                slice_shape = audio_slice.shape
            ...
            if len(audio_slice) < 1:
                ...
                if len(audio_slice.shape) == 1:
                    print(self.videoid2gameid_eventid[audio_id])
                    continue
                # print("!!!!!!!!!!! {}, {}".format(video[i][:slice_shape[0]].shape, video_slice.shape))
        ...
        n_tokens = int(self.max_frames*(1-self.mask_prob))
        trimmed_audio = np.zeros((len(choice_audio_ids), n_tokens, self.audio_dim))
        for i, audio_pair_ in enumerate(masked_audio):
            ...
            #     else:
            #         video_labels_index[i].append(-1)
            unmasked_index = random.sample(range(max_audio_length[i]), n_tokens)
            unmasked_index.sort()
            trimmed_audio[i] = audio_pair_[unmasked_index]
            ...
            for j, _ in enumerate(audio_pair_):
                if j < max_audio_length[i]:
                    ...
                    audio_labels_index[i].append(-1)
        audio_labels_index = np.array(audio_labels_index, dtype=int)
        # -----> Mask Frame Model
        masked_audio = masked_audio > 0
        zeros = np.zeros((masked_audio.shape[0], masked_audio.shape[1]))
        zeros[masked_audio[:,:,0] > 0] = 1
        masked_audio = zeros
        return trimmed_audio, audio_mask, masked_audio, audio_labels_index,audio
     
    def __getitem__(self, idx):
        video_id, caption = self.sentences_dict[idx]

        trimmed_video, video_mask, masked_video, video_labels_index, video = self._get_video([video_id])
        trimmed_audio, audio_mask, masked_audio, audio_labels_index, audio = self._get_audio([video_id])
        ...
        return pairs_text, pairs_mask, pairs_segment, video, video_mask, \
               ...
               pairs_input_caption_ids, pairs_decoder_mask, pairs_output_caption_ids,task_type, audio, audio_mask, masked_audio, trimmed_video, trimmed_audio
