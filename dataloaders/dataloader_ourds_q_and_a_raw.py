...
class OURDS_QA_RAW_DataLoader(Dataset):
    ...
    def __init__(
        ...
        self.raw_frames_path = videos_filepath
        self.video_path = videos_filepath
        assert self.video_path is not None
        # get video paths
        self.video_paths = os.listdir(self.video_path)
        print("Number of raw videos: ", len(self.video_paths))
        # make the path list into ia dictionary of video ids
        self.video_paths = {video_path.split('.')[0]: video_path for video_path in self.video_paths}
        ...
        choiced_video_ids = {vid:1  for vid in choiced_video_ids if vid.replace("video","") in self.video_paths}
    ...
    def _get_rawvideo(self, choice_video_ids):
        # load the mp4 file into a tensor
        video = torch.zeros(len(choice_video_ids), 3, self.max_frames, 224, 224)
        video_mask = torch.zeros((len(choice_video_ids), self.max_frames), dtype=int)
        max_video_length = [0] * len(choice_video_ids)
        
        for i, video_id in enumerate(choice_video_ids):
            raw_video, vid_a, vid_meta = torchvision.io.read_video(self.raw_frames_path + '/' + self.video_paths[video_id.replace("video","")], pts_unit='sec')
            raw_video = raw_video[::2]
            slice_shape = raw_video.shape
            max_video_length[i] = max_video_length[i] if max_video_length[i] > slice_shape[0] else slice_shape[0]
            raw_video = raw_video[:self.max_frames].permute(0, 3, 1, 2)
            video[i][:,:raw_video.shape[0]] = preprocess(raw_video).permute(1, 0, 2, 3)
            
        for i, v_length in enumerate(max_video_length):
            # Ensure that v_length is an integer and calculate the minimum between v_length and self.max_frames
            v_length_min = int(np.min([v_length, self.max_frames]))
            # Use torch.ones to create a tensor of ones with the required length and the same dtype as video_mask
            ones_tensor = torch.ones(v_length_min, dtype=torch.long)
            # Assign this tensor to the appropriate part of video_mask
            video_mask[i][:v_length_min] = ones_tensor
        
        # Mask Frame Model <-----
        video_labels_index = [[] for _ in range(len(choice_video_ids))]
        video_labels_index = np.array(video_labels_index, dtype=int)
        masked_video = torch.zeros(len(choice_video_ids), 3, self.max_frames, 224, 224)
    ...
    def __getitem__(self, idx):
        ...
        return pairs_text, pairs_mask, pairs_segment, video, video_mask, \
               ...
               pairs_input_caption_ids, pairs_decoder_mask, pairs_output_caption_ids,task_type, bbx, bbx_mask, masked_bbx, bbx_labels_index
