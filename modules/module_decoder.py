...
class DecoderEmbeddingsUnsupPretrain(nn.Module):
    """Construct the embeddings from word, position and token_type embeddings.
    """
    def __init__(self, config, decoder_position_embeddings_weight):
        super(DecoderEmbeddingsUnsupPretrain, self).__init__()
        self.position_embeddings = nn.Embedding(config.max_target_embeddings, config.hidden_size)
        self.position_embeddings.weight = decoder_position_embeddings_weight
...
class DecoderModel(PreTrainedModel):
    ...
    def __init__(self, config, decoder_word_embeddings_weight, decoder_position_embeddings_weight, output_concat=False, args=None):
        ...
        if not output_concat:
            self.embeddings = DecoderEmbeddings(config, decoder_word_embeddings_weight, decoder_position_embeddings_weight)
        else:
            self.embeddings = DecoderEmbeddingsUnsupPretrain(config, decoder_position_embeddings_weight)
    ...
    def forward(self, input_ids, encoder_outs=None, answer_mask=None, encoder_mask=None,task_type=None, is_embedding=False):
        """
        Args:
        ...
        if self.args is not None and self.args.unsup_pretrain:
            self_attn_mask = torch.ones((len_s, len_s), device=embedding_output.device, dtype=embedding_output.dtype).unsqueeze(0).expand(sz_b, -1, -1).unsqueeze(1)
        else:
