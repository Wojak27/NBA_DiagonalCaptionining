...
class BertTokenizer(object):
    ...
    def __init__(self, vocab_file, do_lower_case=True, max_len=None, never_split=("[UNK]", "[SEP]", "[MASK]", "[CLS]")):
        ...
        # if key contains "unused" replace it with "action"
        # all_players = json.load(open('all_players.json', 'r'))
        
        # self.vocab = {k if not "unused" in k else all_players[v].lower() if len(all_players) > v else k : v for k, v in self.vocab.items()}
