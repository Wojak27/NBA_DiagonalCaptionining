...
class MaskedMSELoss(nn.Module):
    ...
    def forward(self, input, target, mask=None):
        ...
        if mask is not None:
            assert input.shape[:2] == mask.shape, "Input and mask must have the same shape"
        ...
        if mask is not None:
            ...
            loss = loss.flatten(start_dim=1).sum(dim=1) * mask.flatten(start_dim=1).sum(dim=1) / mask.shape[0]/ mask.shape[1] /input.shape[-1] # <- this could be 0 if mask is 0
