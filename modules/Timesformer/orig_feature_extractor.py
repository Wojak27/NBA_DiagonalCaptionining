...
model = TimeSformer(
    ...
    pretrained_model="/cold2TB/datasets/NSVA/Timesformer/models/TimeSformer_divST_32x32_224_HowTo100M.pyth",
).cuda()
...
def process_videos_in_parallel(tasks, num_processes=8):
...
if __name__ == "__main__":
    ...
    for f in vid_folders:
        try:
            v = video_dict[f.split('.')[0]]
            if f not in store_dir:
                tasks.append((f, "/cold2TB/datasets/NSVA/features/unlabeled_TSF", "/cold2TB/datasets/NSVA/tools/pbp_videos"))
        except KeyError:
            print(f"File not in files dict {f}")
