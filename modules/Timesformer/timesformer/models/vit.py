...
from modules.Timesformer.timesformer.models.vit_utils import IMAGENET_DEFAULT_MEAN, IMAGENET_DEFAULT_STD
from modules.Timesformer.timesformer.models.helpers import load_pretrained
from modules.Timesformer.timesformer.models.vit_utils import DropPath, to_2tuple, trunc_normal_
...
class VisionTransformer(nn.Module):
    ...
    def forward_time_features(self, x):
        B = x.shape[0]
        ...
        # Reshape to treat the batch and embedding size as one dimension and time as another
        x = x[:,1:,:]
        x = x.reshape(-1, T, x.shape[-1])  # Combines the first two dimensions

        # Apply adaptive average pooling across the 'time' dimension (now the middle dimension)
        # Target size for the 'time' dimension is T
        x = x.transpose(2, 0)
        x = F.adaptive_avg_pool1d(x, B).transpose(2, 0)

        # Reshape back to the desired output shape (B, T, x[-1])
        # This step might need adjustment based on how you want to interpret the dimensions
        # x = x.view(B, T, x[-1])
        x = self.norm(x)
        return x
